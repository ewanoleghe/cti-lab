...
def check_dehashed(email: str):
    ...
    Very conservative detection to avoid false positives from page boilerplate.
    """
    try:
        ...
        has_results_indicators = any(phrase in text for phrase in [
            "found", "result", "results for", "breached", "leak", "database", "password", "hash",
            "compromised", "hit", "match", "dehashed found"
        ])

        has_no_results_indicators = any(phrase in text for phrase in [
            "no results", "0 results", "nothing found", "no matches", "no hits",
            "no data", "no breach", "clean", "not found in our database"
        ])

        if has_no_results_indicators:
            print(f"[DeHashed] {email} → explicit no-results signal")
            return 0, []

        if has_results_indicators and not has_no_results_indicators:
            print(f"[DeHashed] {email} → potential hit detected")
            return 1, ["DeHashed potential hit"]

        print(f"[DeHashed] {email} → no clear breach indicators")
        return 0, []
